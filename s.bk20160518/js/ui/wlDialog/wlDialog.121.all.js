	...
	$.extend(m,{
		...
		_create:function(c){
			...
			$.each(c.buttons,function(i,b){
				...
				b.$btn = $('<div class="wlDialog-button"></div>').addClass(b.class)
						.html(b.text).click(function(){
		...
		confirm:function(content,title,cb){
			...
			$.extend(wld,{
				yes:function(f){
					return wld.buttonClick(function(btn){
						f();
					},'ok');
				},
				no:function(f){
		...
		info:function(msg,type,timeout){
			...
			if(timeout){
				setTimeout(function(){ wld.close();}, timeout);
			}
			return wld;
		},
		/* 成功提示框 */
