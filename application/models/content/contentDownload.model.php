...
class ContentArticleModel extends ModelBase{
	var $_name ='content';
	var $_tablename = 'content_download';
	...
	function getArticle($aid='',$cont_id=''){
		...
		$parms = array(
				...
				'join'=>'belong_one_content,belong_one_column,has_thum_image',
				'order_by'	=>	'A.up_time',
				'where'	=>	$where
		);
	 ...
	 */
	function getArticleList($cid=0,$page=1,$pagesize=20){
	    $parms = array(
		...
		if($cid)$parms['where'] = array('B.c_id'=>$cid);
