...
class TreeLib extends Object
{
	 ...
	 */
	function getOptions2($layer = 0, $root = 0, $except = NULL, $space = "&nbsp;-&gt;&nbsp;")
	{
		$options = array();
		...
		foreach ($childs as $id)
		{
			if ($id > 0 && ($layer <= 0 || $this->getLayer($id) <= $layer))
			{
				...
				$options[$id] = $this->getParentsPath($id, $space) . htmlspecialchars($this->getValue($id));
				//var_dump($this->getParentsPath($id, $space));
			}
		}
		return $options;
	}
	function getParentsPath($id,$glue = "&nbsp;-&gt;&nbsp;"){
		$path = '';

		$parents = $this->getParents($id);
		foreach($parents as $k=>$v){
			if($v<=0)continue;
			$path .= $this->getValue($v).$glue;
		}
